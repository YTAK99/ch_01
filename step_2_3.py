...
def dump_dirnames(base_dir: Path) -> None:              # 폴더 목록을 저장하는 함수 dump_dirnames()를 정의한다. 매개변수 base_dir는 폴더 경로를 저장
    ...
    for path in base_dir.iterdir():                     # base_dir 경로에 있는 모든 파일을 반복 처리. 함수 iterdir()는 주정지 높ㄹ더의 모든 파일과 하위 폴더 목록을 반환.
        if path.is_dir():                               # path 변수에 저장된 경고라 유효하면 코드 11행을 실행
    ...
    with open(OUT_2_3, "w", encoding="utf-8") as fp:                        # OUT_2_3 경로에 쓰기 모모드로 파일을 열고, 이를 변수 fp에 저장
...
def load_dirnames() -> list[str]:                                   # 폴더 목록이 저장된 JSON파일을 불러오는 함수 load_dirnames()를 정의
    if OUT_2_3.is_file():                                           # OUT_2_3 경로에 파일이 있는지 확인
        with opne(OUT_2_3, encoding="utf-8") as fp:                 # OUT_2_3 경로에 있는 파일을 불러옴
